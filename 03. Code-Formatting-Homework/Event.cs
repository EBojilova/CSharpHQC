...
class Event:IComparable  {
	...
	CompareTo(object obj)
	{
		Event other    = 
		obj as                 Event;
		int comparingDate     = this.date. CompareTo   (other.date);
		int comparingTitle    = this.title.CompareTo   (other.title);

		int comparingLocation;
		comparingLocation = this.location.CompareTo(other.location);
		if (comparingDate == 0)
		{
			if( comparingTitle==0 )
...
class Program
{
	...
	class EventHolder
	{
		MultiDictionary<string, Event> 
			comparingTitle = new MultiDictionary<string, 
			Event>(true);
		OrderedBag<Event> 
			comparingDate = new OrderedBag<
			Event>();
 							 ...
 							 string location)
		{
			...
			comparingDate.Add(newEvent);	Messages.EventAdded();
		}
		...
		public void DeleteEvents(string titleToDelete)
		{
			string title = titleToDelete			
					.ToLower();
			int removed = 0;               
			foreach ( var eventToRemove in comparingTitle [ title ] ) {		
				removed ++;
				comparingDate.Remove ( eventToRemove );
			}
			comparingTitle.Remove (title);
			Messages.EventDeleted( removed );               
		}
		public void ListEvents(DateTime date, int count)
		{
			OrderedBag<Event>.View 
					eventsToShow = comparingDate.RangeFrom(new Event(
					date, "", ""), 
					true);
			int showed = 0;
